...
import { StoreSchema, useStoreArray } from "./DataStory/store/store";
...
export function DataStoryApp({ defaultFlowName, flow }: { defaultFlowName?: string; flow?: any }) {
  ...
  const { flowName, setFlowName, open } = useStoreArray[0](selector, shallow);
  ...
  return (
    <>
      <div className="w-full h-screen bg-vsCodeWarmGray-900">
        ...
        <div className="w-full h-5/6">{<DataStory id={0}/>}</div>
      </div>
    </>
  );
}
